...
def get_caldav_url(user):
    """Helper function to get normalized CalDAV URL"""
    return build_caldav_principal_url(user.caldav_url, user.caldav_username)

@frappe.whitelist()
def get_calendar(nuser):
    """Fetch available calendars for a user"""
    fp_user = frappe.get_doc("User", nuser)
    if not (fp_user.caldav_url and fp_user.caldav_username and fp_user.caldav_token):
        frappe.msgprint(_("Please configure CalDAV settings in your User profile"))
        return []
    
    caldav_url = get_caldav_url(fp_user)
...
def sync_caldav_event_by_user(doc, method=None):
    """Sync Frappe event to CalDAV server"""
    ...
    if not doc.sync_with_caldav:
        if doc.event_uid:
            # Call remove_caldav_event directly to ensure proper session context
    ...
    if doc.caldav_id_url:
        ...
        if '_shared_by_' in ocal:
            ...
            ocal = ocal[0:pos]
        # Only remove event if calendar changed
        if doc.caldav_id_calendar and ocal not in doc.caldav_id_calendar:
            remove_caldav_event(doc)
            doc.caldav_id_url = None
            doc.event_uid = None
            doc.event_stamp = None
    ...
    caldav_calendar_url = doc.caldav_id_calendar or doc.caldav_id_url
    ...
    if not caldav_calendar_url:
        frappe.msgprint(_("Please select a CalDAV calendar"))
        return
    
    ucal = str(caldav_calendar_url).split("/")
    cal_name = ucal[-2]  # Get Calendar Name from URL
    
    # Get CalDav connection details
    caldav_url = get_caldav_url(fp_user)
    caldav_username = fp_user.caldav_username
    ...
    if not calendars:
        frappe.msgprint(_("No calendars found on CalDAV server"))
        return
    ...
    for c in calendars:
        ...
        if str_cal == cal_name or (str_cal + "_shared_by_") in str(caldav_calendar_url or ""):
            ...
            dtstart = datetime.strptime(doc.starts_on, '%Y-%m-%d %H:%M:%S') if isinstance(doc.starts_on, str) else doc.starts_on
            if doc.all_day:
            ...
            if doc.ends_on:
                dtend = datetime.strptime(doc.ends_on, '%Y-%m-%d %H:%M:%S') if isinstance(doc.ends_on, str) else doc.ends_on
                if doc.all_day:
            ...
            if doc.repeat_this_event and doc.repeat_on:
                if not doc.repeat_till:
                    ...
                    dtuntil = datetime.strptime(doc.repeat_till, '%Y-%m-%d') if isinstance(doc.repeat_till, str) else doc.repeat_till
            ...
            try:
                if is_new_event:
                    ...
                    # UPDATE: delete + recreate (SOGo-safe)
                    try:
                        evobj = c.event_by_url(str(c.url).rstrip("/") + "/" + uidstamp + ".ics")
                        evobj.delete()
                    except Exception:
                        pass  # Event doesn't exist, will be created below
...
def remove_caldav_event(doc, method=None):
    """Remove event from CalDAV server"""
                ...
                try:
                    ...
                    if not event_deleted:
                        try:
                            ...
                            for url_event in recent_events[:50]:  # Limit to 50 events
                                try:
                                    cal_url = str(url_event).replace("Event: https://", "https://" + caldav_username + ":" + caldav_token + "@")
                                    req = requests.get(cal_url, timeout=2)
            ...
            for c in calendars:
                try:
                    ...
                    for url_event in sel_events:
                        try:
                            # Use event.data directly instead of separate HTTP request
                            ...
                            for evento in cal.walk('vevent'):
                                try:
                                    ...
                                    if fp_event:
                                        ...
                                        # CalDAV dtstamp always to UTC
                                        if caldav_stamp.tzinfo:
                                            caldav_stamp_utc = caldav_stamp.astimezone(UTC)
                                        else:
                                            caldav_stamp_utc = UTC.localize(caldav_stamp)
                                        
                                        if is_event_modified(
                                            fp_event[0].event_stamp,
                                            caldav_stamp_utc.strftime("%Y-%m-%d %H:%M:%S")
                                        ):
...
def prepare_fp_event(event, cal_event, user_tz=None):
    """Prepare event for Frappe from CalDAV event"""
    if not user_tz:
        user_tz = get_user_timezone()
    ...
    if dtstamp.tzinfo:
        dtstamp_utc = dtstamp.astimezone(UTC)
    else:
        dtstamp_utc = UTC.localize(dtstamp)
    event.event_stamp = dtstamp_utc.strftime("%Y-%m-%d %H:%M:%S")
    ...
    if 'rrule' in cal_event:
        ...
        if rule:
            ...
            if 'FREQ' in rrule:
                ...
                if frequency == "Weekly":
                    if 'BYDAY' in rrule:
                        event.monday = 'MO' in rrule['BYDAY']
                        event.tuesday = 'TU' in rrule['BYDAY']
                        event.wednesday = 'WE' in rrule['BYDAY']
                        event.thursday = 'TH' in rrule['BYDAY']
                        event.friday = 'FR' in rrule['BYDAY']
                        event.saturday = 'SA' in rrule['BYDAY']
                        event.sunday = 'SU' in rrule['BYDAY']
            if 'UNTIL' in rrule:
